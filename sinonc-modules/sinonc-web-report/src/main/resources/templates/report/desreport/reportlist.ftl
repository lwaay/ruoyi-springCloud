...
<title>在线设计</title>
    ...
    <link rel="stylesheet" href="${base}${customPrePath}/jmreport/desreport/corelib/cust.css">
    ...
    <div class="layout" style="margin-left: -30px;margin-top: -10px;">
        <Layout>
            <Sider breakpoint="md" collapsible :collapsed-width="78" v-model="isCollapsed">
                ...
                <i-menu theme="dark" width="auto" :class="menuitemClasses" active-name="printinfo" :open-names="['sub']" @on-select="onMenuSelect">
                    <Submenu name="sub">
                        <template slot="title">
                ...
                <tab-pane icon="md-desktop" label="报表设计" name="name1">
                    ...
                    <div style="display: flex;flex-wrap: wrap;">
                        ...
                        <div
                            ...
                            <!-- 缩略图 &ndash;&gt;-->
                            <div class="thumb">
                                <img src="${base}${customPrePath}/jmreport/desreport/corelib/jiade.jpg"/>
...
<script>
    ...
    var currentPage = new Vue({
        ...
        mounted:function(){
           ...
           this.actionUrl=BASE_URL+"/jimu/putFile";
           this.$nextTick(()=>{
                this.dataSource=[];
                this.userInfos();
            });
        ...
        methods: {
                ...
                for (var i = 0; i < ca.length; i++) {
                    ...
                    if (c.indexOf(name) != -1){
                       this.token=c.substring(name.length, c.length);
                        window.localStorage.setItem('JmReport-Access-Token',this.token);
                    }
                }
                return "";
            },
            ...
            createExcel: function(){
                ...
                $http.post({
                    ...
                    contentType:'json',
                    success:(result)=>{
                        window.open(apis.index+result.id+"?token="+this.token);
                    }
                },that)
            },
            //未使用
            handleEditConfig: function(item){
                window.location.href = apis.index+item.id+"?token="+this.token;
            },
            //删除报表
            handleDelete:function(item){
                $http.confirm({
                    content:'是否确认删除?',
                    url:apis.deleteReport,
                    data:{
                        id:item.id,
                        token:this.token
                    },
            ...
            getExcelEditUrl: function(item){
                return apis.index+item.id+"?token="+this.token;
            },
            getExcelViewUrl: function(item){
                return apis.view+item.id+"?token="+this.token;
            },
