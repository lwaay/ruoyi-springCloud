...
// 预览页面 vue实例
 ...
 view.loads = function(excelConfigId){
     ...
     if(jmdata){
        ...
        xs.loadData(excelData);
         ...
         let total = 1;
         options.pageInfo={
             url:`${view.origin}${baseFull}/jimu/show`,
             data:{
                 "id":excelConfigId,
                 "params":requestParam
             }
         ...
         var loadExcelView = function(inputparam){
             ...
             $jm.excelView(excelConfigId,requestParam,(result)=> {
                 ...
                 console.info("test",dataMap);
                     ...
                     options.getAll=`${view.origin}${baseFull}/jimu/show?id=${excelConfigId}&params=${printAllFlag}`;
                     //查找全部数据接口
                     ...
                     xs = x.spreadsheet('#x-spreadsheet-demo', options)
                         .onExportExcelPage(function(){
                             ...
                             //window.open(baseFull+`/jmreport/exportExcel/${excelConfigId}/${settings.page}/${pageSize}/${requestStr}`);
                         })
                     ...
                     if(finalJsonObj.chartList && finalJsonObj.chartList.length >0){
                         finalJsonObj.chartList.forEach(function(item){
                             ...
                             if (config.geo){
                                 $http.post({
                                     contentType:'json',
                                     url:apis.queryMapByCodeUseOrigin,
